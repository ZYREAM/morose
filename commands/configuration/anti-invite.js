...
module.exports = {
    ...
    async execute(client, message, args, droit) {
        ...
        if(args[0] === "on"){
            ...
            const droit = new Discord.MessageEmbed()
            .setColor('#9b9b9b')
                .setDescription("<a:oui:873277851695206401> La pub est désormais interdite (sauf dans le salon pub ou si vous avez la permission [gérer les messages])");
                message.lineReplyNoMention(droit);
        }else if(args[0] === "off"){
            srv.set(`${message.guild.id}`, "off", "antipub");
            const droit = new Discord.MessageEmbed()
            .setColor('#9b9b9b')
                .setDescription("<a:oui:873277851695206401> La pub est désormais autorisée");
                message.lineReplyNoMention(droit);
        }else {
            const droit = new Discord.MessageEmbed()
            .setColor('#9b9b9b')
