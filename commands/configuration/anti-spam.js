...
module.exports = {
    ...
    if(args[0] === "on"){
        srv.set(`${message.guild.id}`, "on", "statspam");
        const droit = new Discord.MessageEmbed()
        .setColor('#9b9b9b')
            .setDescription("<a:oui:873277851695206401> L'antispam est ON");
            message.lineReplyNoMention(droit);
    }else if(args[0] === "off"){
        srv.set(`${message.guild.id}`, "off", "statspam");
        const droit = new Discord.MessageEmbed()
        .setColor('#9b9b9b')
            .setDescription("<a:oui:873277851695206401> L'antispam est OFF");
            message.lineReplyNoMention(droit);
    }else {
        const droit = new Discord.MessageEmbed()
        .setColor('#9b9b9b')
